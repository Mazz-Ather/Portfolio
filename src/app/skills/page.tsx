...
const Skill = ({ name, x, y }: { name: string; x: string; y: string }) => {
     return(
     <motion.div className="flex items-center justify-center rounded-full font-semibold bg-black text-white px-5 
...
<Navbar/>
        ...
        <h2 className="!bg-white font-bold text-8xl -mb-4 w-full flex items-center justify-center text-black ">Skills</h2>
   <div className="!bg-white">

        <div className=" w-full h-[85vh] relative flex items-center justify-center rounded-full bg-circularLight -mb-1">
          {/* Additional content */}
          <motion.div className="flex items-center justify-center rounded-full font-semibold bg-black text-white p-5 cursor-pointer shadow-gray-600"
          whileHover={{scale:1.05}}
          >
               Web
